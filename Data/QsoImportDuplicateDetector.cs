...
internal static class QsoImportDuplicateDetector
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var existingKeys = await db.Qsos
            .AsNoTracking()
            .Select(x => new DuplicateProbe(x.Call, x.StationCall, x.QsoDate, x.Band, x.Mode, x.Freq))
            .ToListAsync(cancellationToken);
        ...
        {
            var signature = CreateSignature(new DuplicateProbe(qso.Call, qso.StationCall, qso.QsoDate, qso.Band, qso.Mode, qso.Freq));
    ...
    private static string CreateSignature(DuplicateProbe qso)
    {
        ...
        var stationCall = NormalizeText(qso.StationCall);
        ...
        return string.Join("|", call, stationCall, date, band, mode, freq);
    }
    ...
    private readonly record struct DuplicateProbe(string Call, string StationCall, DateTime QsoDate, string Band, string Mode, decimal Freq);
}
